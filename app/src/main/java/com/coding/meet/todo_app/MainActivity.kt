...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val buttonOpenWeather: ImageView = findViewById(R.id.ic_weather)
        ...
        val taskRVVBListAdapter =
            TaskRVVBListAdapter(isListMutableLiveData) { type, _, task ->
                if (type == "delete") {
                    taskViewModel
                        // Deleted Task
//                .deleteTask(task)
    ...
    private fun restoreDeletedTask(deletedTask: Task) {
        val snackBar = Snackbar.make(
            mainBinding.root, "Удалена  '${deletedTask.title}'",
            Snackbar.LENGTH_LONG
        )
        snackBar.setAction("Назад") {
    ...
    private fun callSortByDialog() {
        ...
        val items = arrayOf("а-я", "я-а", "Сортировать по новизне", "Сортировать по старизне")
                ...
                when (it.status) {
                    ...
                    Status.SUCCESS -> {
                        ...
                        when (it.data as StatusResult) {
                            Added -> {
                                Log.d("StatusResult", "Добавлена")
                            }

                            Deleted -> {
                                Log.d("StatusResult", "Удалена")

                            }

                            Updated -> {
                                Log.d("StatusResult", "Обнавлена")
    ...
    private fun callGetTaskList(taskRecyclerViewAdapter: TaskRVVBListAdapter) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            taskViewModel
                .taskStateFlow
                .collectLatest {
                    Log.d("Статус", it.status.toString())
